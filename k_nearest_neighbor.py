...
         предсказать метку для каждой контрольной точки.

         Входы:
         - dists: массив пустых фигур (num_test, num_train), где dists [i, j]
           дает расстояние между i-й контрольной точкой и j-й контрольной точкой.
...
           тестовые данные, где y [i] - предсказанная метка для контрольной точки X [i].
        ...
        for i in range(num_test):
            ...
            """
            # Используйте матрицу расстояний, чтобы найти k ближайших соседей i-й точки
            тестирования, и используйте self.y_train, чтобы найти метки этих соседей.
            Храните эти ярлыки в closest_y. #
            # Подсказка: найдите функцию numpy.argsort.
            """
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            #возвращает список индексов отсортированных элементов массива.
            # аргосрт нумерует элементы, затем сортирует по возрастанию и возвращает индексы
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            # bincount вовзращает кол-во вхождений каждого элемента
            # после чего находим максимальное число вхождений и возвращаем
